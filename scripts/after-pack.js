...
async function afterPack(context) {
  ...
  try {
    ...
    // Verify dist-backend is properly unpacked for runtime access
    const asarPath = path.join(appOutDir, 'resources', 'app.asar');
    const distBackendPath = path.join(appOutDir, 'resources', 'app.asar.unpacked', 'dist-backend');
    ...
    if (fs.existsSync(distBackendPath)) {
      console.log('✅ dist-backend found in unpacked directory - backend executables are accessible');
      
      // Verify the required executables exist
      const files = fs.readdirSync(distBackendPath);
      console.log('📁 Backend executables found:', files.join(', '));
      
      // Check for Windows executable specifically
      if (fs.existsSync(path.join(distBackendPath, 'server.exe'))) {
        console.log('✅ Windows backend executable (server.exe) verified');
      } else {
        console.warn('⚠️  Windows backend executable (server.exe) not found!');
      }
    } else {
      console.error('❌ dist-backend not found in unpacked directory - backend will not work!');
    }
