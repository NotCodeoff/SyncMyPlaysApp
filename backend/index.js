...
// Use regular console (secure console files don't exist)
const secureConsole = console;
const secureLogger = {
...
try {
  ...
  try {
    ...
    logger.error('Puppeteer not available');
  }
}
...
try {
  ...
  try {
    ...
    logger.warn('Could not load PuppeteerConfig, using default configuration');
...
class SimpleStore {
  constructor() {
    ...
    if (process.env.NODE_ENV === 'production' || process.pkg) {
      ...
      try {
        ...
        logger.warn('Could not create data directory, falling back to current directory:', error.message);
        dataDir = __dirname;
      }
    } else {
  ...
  load() {
    try {
      ...
      logger.error('Error loading store:', error.message);
    }
    return {};
  }
  ...
  save() {
    try {
      ...
      logger.error('Error saving store:', error.message);
    }
  }
...
// Professional logging system is now handled by logger.js
// No need for manual file stream management
...
// Professional API request logging
...
function broadcast(payload) {
  ...
  logger.info('[BROADCAST]', messageString);
  for (const ws of activeConnections) {
    try {
      ws.send(messageString);
    } catch (error) {
      logger.error('WebSocket send error:', error && error.stack ? error.stack : error);
    }
  }
}
...
async function validateSyncResults(playlistId, headers, expectedAdded, totalTracks) {
  try {
    ...
    logger.info(`Validation: Expected ${expectedAdded} added, Actual ${actualAdded} in playlist`);
    
    return { actualAdded, actualNotAdded };
  } catch (error) {
    logger.error('Validation failed, using estimated counts:', error);
...
async function requestWithRetry(requestFn, label, backoffMs = [1000, 3000, 5000]) {
  ...
  while (true) {
    try {
      ...
      logger.error(`[${label}] Request failed (attempt ${attempt + 1}):`, {
        status,
        data,
      ...
      if (attempt < backoffMs.length && isRetriable) {
        ...
        logger.warn(`[${label}] Retrying after ${wait}ms due to status ${status}`);
        await delay(wait);
        attempt += 1;
        continue;
      }
...
const makeAppleMusicApiRequest = async (url, options = {}) => {
  try {
    ...
    if (error.response && error.response.status >= 500) {
      logger.info(`[AppleMusic] Retrying once after 500ms due to status ${error.response.status}...`);
      // No delay - instant speed
      return await axios({
        url,
        ...options,
...
const makeSpotifyApiRequest = async (url, options = {}) => {
  try {
    ...
    if (spotifyRequestCount >= SPOTIFY_RATE_LIMIT) {
      ...
      logger.info(`[SpotifyAPI] Rate limit approaching, waiting ${waitTime}ms...`);
...
async function getDeveloperToken(forceRefresh = false) {
  ...
  tokenFetchPromise = (async () => {
    try {
      ...
      logger.info('Apple Music developer token required for integration.');
      logger.info('To set up Apple Music integration:');
      logger.info('1. Visit: https://developer.apple.com/account/resources/authkeys/list');
      logger.info('2. Sign in with your Apple ID');
      logger.info('3. Click "Generate API Key" or use an existing key');
      logger.info('4. Copy the generated token');
      logger.info('5. Use the "Setup Apple Music Token" button in the app UI');
      logger.info('6. Or call POST /auth/apple/set-developer-token with your token');
      logger.info('');
      logger.info('💡 Alternative: Set APPLE_MUSIC_DEVELOPER_TOKEN environment variable');
      logger.info('💡 This simple approach requires no additional software installations');
...
async function generateUniquePlaylistName(baseName, service = 'apple') {
  ...
  try {
    ...
    logger.info(`Generated unique playlist name: "${uniqueName}" (original: "${trimmedName}")`);
    return uniqueName;
    
  } catch (error) {
    logger.warn('Failed to generate unique playlist name, using original:', error.message);
    return trimmedName;
  }
}
...
async function runSpotifyToAppleSync(spotifyPlaylistId, applePlaylistId, storefront = 'us') {
  ...
  logger.debug(`Destination playlist has ${destExistingIds.size} existing tracks`);
  ...
  logger.info(`Starting parallel batch processing of ${tracks.length} songs in batches of ${BATCH_SIZE}...`);
  ...
  for (let batchStart = 0; batchStart < tracks.length; batchStart += BATCH_SIZE) {
    ...
    logger.info(`Processing batch ${batchNumber}/${totalBatches} (${batch.length} songs)...`);
    ...
    batchResults.forEach(({ result, track, success, error }) => {
      if (success && result && result.success && result.match) {
        ...
        logger.debug(`DEBUG: Checking duplicate for "${track.name}"`);
        logger.debug(`DEBUG: appleId: ${appleId}`);
        logger.debug(`DEBUG: result.match.id: ${result.match.id}`);
        logger.debug(`DEBUG: result.match.catalogId: ${result.match.catalogId || 'none'}`);
        logger.debug(`DEBUG: result.match.libraryId: ${result.match.libraryId || 'none'}`);
        logger.debug(`DEBUG: destExistingIds size: ${destExistingIds.size}`);
        logger.debug(`DEBUG: isDuplicate: ${isDuplicate}`);
        
        if (isDuplicate) {
          stats.skipped++;
          logger.info(`Skipping duplicate: "${track.name}" by ${track.artists?.[0]?.name || 'Unknown'} (already in destination)`);
          logger.debug(`DEBUG: Apple ID ${appleId} found in existing set (size: ${destExistingIds.size})`);
          ...
          logger.info(`Adding new song: "${track.name}" by ${track.artists?.[0]?.name || 'Unknown'} (Apple ID: ${appleId})`);
          logger.debug(`DEBUG: Apple ID ${appleId} NOT found in existing set (size: ${destExistingIds.size})`);
          ...
          logger.info(`${confidence} Matched: ${track.name} (${result.match.matchMethod}, ${result.match.matchTime}ms)`);
        }
      } else {
        stats.unavailable++;
        logger.error(`Unavailable: ${track.name} by ${track.artists?.[0] || 'Unknown'}`);
    ...
    logger.info(`Batch ${batchNumber}/${totalBatches} complete: ${batchResults.filter(r => r.success && r.result && r.result.success).length} matched, ${batchResults.filter(r => !r.success || !r.result || !r.result.success).length} not found`);
  }
  
  logger.info(`Parallel processing complete: ${stats.autoMatched} matched, ${stats.skipped} skipped (already existed), ${stats.unavailable} unavailable`);

  // Enhanced: Add missing songs in correct order positions
  if (toAdd.length > 0) {
    logger.info(`Smart insertion: Adding ${toAdd.length} missing songs in correct order positions...`);
    ...
    logger.info(`Smart insertion complete: Added ${toAdd.length} songs in correct order positions`);
  } else {
    logger.info(`No missing songs to add - all songs already exist in destination playlist`);
  }
  
  // Update final statistics to include skipped count
  logger.info(`Spotify→Apple sync completed: ${toAdd.length} added, ${stats.autoMatched} auto-matched, ${stats.skipped} skipped (already existed), ${stats.unavailable} unavailable`);
  ...
  logger.info(`Sync Complete: ${stats.autoMatched} transferred, ${stats.unavailable} unavailable, ${stats.ignored} skipped`);
  return { 
    added: toAdd.length, 
    total: tracks.length,
    stats: stats
...
async function fetchApplePlaylistTrackIdsOnly(playlistId, headers) {
  ...
  logger.debug(`Fetched ${catalogIds.size} existing track IDs from Apple Music playlist`);
  
  // Debug: Log first few IDs to see format
  const idArray = Array.from(catalogIds);
  if (idArray.length > 0) {
    logger.debug(`DEBUG: Sample existing Apple IDs: ${idArray.slice(0, 3).join(', ')}`);
  }
...
async function fetchSpotifyPlaylistTrackIdsOnly(playlistId) {
  ...
  logger.debug(`Fetched ${ids.length} existing track IDs from destination playlist`);
  return new Set(ids);
}
...
async function runAppleToSpotifySync(applePlaylistId, spotifyPlaylistId) {
  ...
  logger.info(`Starting FAST parallel batch processing of ${sourceTracks.length} Apple Music songs in batches of ${BATCH_SIZE}...`);
  ...
  for (let batchStart = 0; batchStart < sourceTracks.length; batchStart += BATCH_SIZE) {
    ...
    logger.info(`Processing Apple→Spotify batch ${batchNumber}/${totalBatches} (${batch.length} songs)...`);
    ...
    const batchPromises = batch.map(async (t, index) => {
      ...
      try {
        ...
        // logger.info(`Raw Apple track data:`, {
        //   name: t.name,
        //   title: t.title,
        ...
        try {
          // logger.info(`Enhanced matcher failed for "${t.name}", trying simple search...`);
          ...
          logger.info(`Simple fallback also failed for "${t.name}": ${fallbackError.message}`);
        }
    ...
    batchResults.forEach(({ result, track, success, error, index }) => {
      if (success && result && result.success && result.id) {
        ...
        if (destExistingIds.has(spotifyId)) {
          stats.skipped++;
          logger.info(`Skipping duplicate: "${track.name}" by ${track.artistName} (already in destination)`);
          ...
          logger.info(`Adding new song: "${track.name}" by ${track.artistName} (Spotify ID: ${spotifyId})`);
  ...
  logger.info(`Apple→Spotify sync completed: ${toAdd.length} added, ${stats.autoMatched} auto-matched, ${stats.skipped} skipped (already existed), ${stats.needsReview} need review, ${stats.unavailable} unavailable, ${stats.ignored} ignored`);
  ...
  for (const job of autoSyncJobs) {
    ...
    if (nextRunAt <= nowIso) {
      try {
        ...
        logger.error('Auto Sync job failed:', e && e.message ? e.message : String(e));
        job.nextRunAt = computeNextRunAtDaily(job.timeOfDay || '16:00');
        persistAutoSyncJobs();
...
async function getPlaylistName(playlistId, headers, service = 'apple') {
  try {
    if (service === 'apple') {
      ...
      logger.info(`getPlaylistName: Apple API response for ${playlistId}:`, JSON.stringify(res?.data, null, 2));
      ...
      if (playlistName) {
        logger.info(`getPlaylistName: Successfully extracted name "${playlistName}" for playlist ${playlistId}`);
        return playlistName;
      } else {
        logger.warn(`getPlaylistName: Could not extract name from Apple API response for ${playlistId}`);
      ...
      // Debug: Log the full response structure
      logger.info(`getPlaylistName: Spotify API response for ${playlistId}:`, JSON.stringify(res?.data, null, 2));
      
      if (res?.data?.name) {
        logger.info(`getPlaylistName: Successfully extracted name "${res.data.name}" for playlist ${playlistId}`);
        return res.data.name;
      } else {
        logger.warn(`getPlaylistName: Could not extract name from Spotify API response for ${playlistId}`);
        return `Playlist ${playlistId}`;
      }
    }
  } catch (error) {
    logger.warn(`getPlaylistName: Failed to get playlist name for ${playlistId}:`, error.message);
    logger.warn(`getPlaylistName: Error details:`, error);
  }
  return `Playlist ${playlistId}`;
}
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        logger.warn('Failed to fingerprint destination playlist via all endpoints. Treating as empty for comparison.');
        return tracks; // empty
      }
    }
  }
...
async function detectAppleStorefront(headers) {
  try {
    ...
    logger.error("Storefront detection failed, falling back to 'us'");
    return 'us';
  }
}
...
async function checkCatalogAvailability(ids, headers, storefront) {
  ...
  for (let i = 0; i < ids.length; i += chunkSize) {
    ...
    try {
      ...
      logger.warn('Availability check failed for chunk; continuing:', e && e.message ? e.message : String(e));
    }
  }
  return available;
}
...
async function fetchCatalogMetadataForIds(ids, headers, storefront = 'us') {
  ...
  for (let i = 0; i < ids.length; i += chunkSize) {
    ...
    try {
      ...
      logger.warn('Catalog metadata fetch failed for chunk; continuing:', e && e.message ? e.message : String(e));
    }
  }
  return result;
}
...
async function fetchSpotifyPlaylistTracks(playlistId) {
  ...
  const basicTracks = rawItems
    .map((it, index) => {
      ...
      if (!track) {
        logger.warn(`Skipping null track at position ${index + 1}`);
        return null;
      }
      ...
      if (!processedTrack.name || processedTrack.name === 'Unknown Track') {
        logger.warn(`Skipping track with missing name at position ${index + 1}`);
        return null;
      }
  ...
  for (let i = 0; i < tracksNeedingIsrc.length; i += perfMode.batchSize) {
    ...
    try {
      ...
      logger.error('Failed to enrich ISRC batch:', error && error.response ? error.response.data : (error && error.message ? error.message : String(error)));
      // If we hit rate limiting, wait longer before continuing
      if (error && error.response && error.response.status === 429) {
        logger.info('Rate limited during ISRC enrichment, waiting 15 seconds...');
        await delay(15000);
      }
    }
  }
...
async function addTracksToApplePlaylistInBatches(playlistId, songIds, headers, options = {}) {
  ...
  for (let i = 0; i < songIds.length; i += BATCH_SIZE) {
    ...
    try {
      ...
      const status = error && error.response ? error.response.status : null;
      const data = error && error.response && error.response.data ? error.response.data : null;
      logger.error('AppleAddTracks batch failed:', {
        playlistId,
      ...
      if (status === 404) {
        ...
        try {
          ...
          logger.error('AppleAddToLibrary fallback failed:', libErr && libErr.response ? libErr.response.data : (libErr && libErr.message ? libErr.message : String(libErr)));
        }
        ...
        if (missingForDetails.length > 0) {
          try {
            ...
            logger.warn('Catalog metadata fetch failed during fallback; searches may be weaker.', metaErr && metaErr.message ? metaErr.message : String(metaErr));
          }
        }
        ...
        for (const id of batch) {
          ...
          try {
            ...
            logger.error('Library resolution failed:', resolveErr && resolveErr.message ? resolveErr.message : String(resolveErr));
          }
          // No delay - instant speed
        }
        ...
        for (const id of batch) {
          ...
          try {
            ...
            logger.error('AppleAddTrackSingle failed:', {
              playlistId,
              songId: id,
              status: s,
              data: d,
...
async function getLibrarySongByFuzzySearch(sourceTrack, headers) {
  ...
  try {
    ...
    logger.error('Library search failed:', error && error.response ? error.response.data : (error && error.message ? error.message : String(error)));
    return null;
  }
}
...
async function fetchApplePlaylistCatalogSongIds(playlistId, headers) {
  ...
  const tryParsePageItems = (items) => {
    for (const item of items || []) {
      ...
      if (item.type === 'songs' && item.id) {
        ...
        logger.debug(`DEBUG: Added song ID: ${item.id} (type: ${item.type})`);
        continue;
      }
      ...
      if (catalogId) {
        ...
        logger.debug(`DEBUG: Added catalog ID: ${catalogId} (type: ${item.type})`);
      }
    }
  };
  ...
  try {
    ...
    if (s1 === 404) {
      logger.warn('AppleFetchPlaylistTracks returned 404; treating playlist as empty for duplicate prevention.', { playlistId });
    ...
    try {
      ...
      if (s2 === 404) {
        logger.warn('AppleFetchPlaylistTracksFallback returned 404; treating playlist as empty for duplicate prevention.', { playlistId });
        return existing;
      }
      ...
      try {
        ...
        logger.warn('Failed to fetch destination playlist catalog song ids via all endpoints; treating as empty set.', {
          playlistId,
          status: s3,
          data: d3,
        });
...
app.get('/callback', async (req, res) => {
  ...
  try {
    ...
    logger.error('Error exchanging code for token:', err.response ? err.response.data : err.message);
...
app.post('/auth/apple/install-dependencies', async (req, res) => {
  try {
    logger.info('Manual dependency installation requested...');
    ...
    logger.error('Manual dependency installation failed:', error.message);
    res.status(500).json({
      success: false,
...
app.post('/auth/apple/open-token-setup', async (req, res) => {
  try {
    ...
    logger.info('Opening Apple Music in browser for token setup...');
    logger.info('Please follow these steps:');
    logger.info('1. Wait for the page to load completely');
    logger.info('2. Press F12 to open Developer Tools');
    logger.info('3. Go to Console tab');
    logger.info('4. Type: MusicKit.getInstance().developerToken');
    logger.info('5. Copy the token value');
    logger.info('6. Use the "Set Developer Token" button in the app');
    ...
    logger.error('Error opening browser for token setup:', error);
    res.status(500).json({ 
...
app.get('/playlists/spotify', async (req, res) => {
  ...
  try {
    ...
    logger.error('Failed to fetch Spotify playlists:', err.response ? err.response.data : err.message);
...
app.post('/playlists/spotify/create', async (req, res) => {
  ...
  try {
    ...
    logger.error('Spotify create playlist error:', err && err.response ? err.response.data : (err && err.message ? err.message : String(err)));
...
app.post('/playlists/apple/create', async (req, res) => {
  ...
  try {
    ...
    logger.error('Apple Music create playlist error:', err && err.response ? err.response.data : (err && err.message ? err.message : String(err)));
...
app.post('/features/join_playlists', async (req, res) => {
  try {
    ...
    logger.error('join_playlists failed:', error && error.response ? error.response.data : error.message);
...
app.post('/features/split_playlist', async (req, res) => {
  try {
    ...
    logger.error('split_playlist failed:', error && error.response ? error.response.data : error.message);
...
app.post('/features/export_favorites', async (req, res) => {
  try {
    ...
    logger.error('export_favorites failed:', error && error.response ? error.response.data : error.message);
...
app.post('/features/export_playlist', async (req, res) => {
  try {
    ...
    logger.error('export_playlist failed:', error && error.response ? error.response.data : error.message);
...
app.post('/ai/user-profile', async (req, res) => {
  try {
    ...
    logger.error('User profile analysis failed:', error.message);
...
app.post('/collaborative/playlists', async (req, res) => {
  try {
    ...
    logger.error('Collaborative playlist creation failed:', error.message);
...
app.get('/collaborative/playlists/:playlistId', async (req, res) => {
  try {
    ...
    logger.error('Get collaborative playlist failed:', error.message);
...
app.put('/collaborative/playlists/:playlistId', async (req, res) => {
  try {
    ...
    logger.error('Update collaborative playlist failed:', error.message);
...
app.post('/collaborative/playlists/:playlistId/collaborators', async (req, res) => {
  try {
    ...
    logger.error('Add collaborator failed:', error.message);
...
app.get('/sync/scheduled', async (req, res) => {
  try {
    ...
    logger.error('Get scheduled sync jobs failed:', error.message);
...
app.post('/sync/scheduled', async (req, res) => {
  try {
    ...
    logger.error('Create scheduled sync job failed:', error.message);
...
app.put('/sync/scheduled/:jobId', async (req, res) => {
  try {
    ...
    logger.error('Update sync job failed:', error.message);
...
app.delete('/sync/scheduled/:jobId', async (req, res) => {
  try {
    ...
    logger.error('Delete sync job failed:', error.message);
...
app.get('/admin/overview', async (req, res) => {
  try {
    ...
    logger.error('Get admin overview failed:', error.message);
...
async function generateAIRecommendations(userId, service, limit, context) {
  try {
    ...
    logger.error('Generate AI recommendations failed:', error.message);
    return [];
  }
}
...
async function analyzeUserProfile(userId, service) {
  try {
    ...
    logger.error('Analyze user profile failed:', error.message);
...
async function getCollaborativeRecommendations(userId, service) {
  try {
    ...
    logger.error('Get collaborative recommendations failed:', error.message);
    return [];
  }
}
...
async function createCollaborativePlaylist(name, ownerId, collaborators, permissions, service, sourcePlaylistId) {
  try {
    ...
    logger.error('Create collaborative playlist failed:', error.message);
    throw error;
  }
}
...
async function getSpotifyTopTracks() {
  try {
    ...
    logger.error('Get Spotify top tracks failed:', error.message);
    return [];
  }
}
...
async function getSpotifyTopArtists() {
  try {
    ...
    logger.error('Get Spotify top artists failed:', error.message);
    return [];
  }
}
...
function computeNextRunTime(schedule) {
  try {
    ...
    logger.error('Compute next run time failed:', error.message);
    // Default: run in 1 hour
...
app.post('/collaborative/sessions/join', async (req, res) => {
  try {
    ...
    logger.error('Join collaborative session failed:', error.message);
...
app.post('/collaborative/sessions/leave', async (req, res) => {
  try {
    ...
    logger.error('Leave collaborative session failed:', error.message);
...
app.get('/admin/performance', async (req, res) => {
  try {
    ...
    logger.error('Get performance metrics failed:', error.message);
...
app.get('/admin/sync-analytics', async (req, res) => {
  try {
    ...
    logger.error('Get sync analytics failed:', error.message);
...
app.post('/users/profile', async (req, res) => {
  try {
    ...
    logger.error('Create/update user profile failed:', error.message);
...
app.put('/users/profile/:userId/preferences', async (req, res) => {
  try {
    ...
    logger.error('Update user preferences failed:', error.message);
...
app.get('/admin/config', async (req, res) => {
  try {
    ...
    logger.error('Get system config failed:', error.message);
...
app.put('/admin/config', async (req, res) => {
  try {
    ...
    logger.error('Update system config failed:', error.message);
...
app.get('/admin/status/realtime', async (req, res) => {
  try {
    ...
    logger.error('Get real-time status failed:', error.message);
...
function initializeCollaborativeStore() {
  try {
    ...
    logger.info('Collaborative store initialized with sample data');
    
  } catch (error) {
    logger.error('Failed to initialize collaborative store:', error.message);
  }
}
...
app.post('/features/dedupe_playlist', async (req, res) => {
  try {
    ...
    if (svc === 'spotify') {
      ...
      logger.info('Deduplication: Spotify using playlist name:', originalPlaylistName);
    ...
    logger.info('Deduplication: Attempting to fetch playlist details for playlist ID:', playlist);
    let originalPlaylistName = playlistName; // Use the name from frontend first
    if (!originalPlaylistName) {
      originalPlaylistName = await getPlaylistName(playlist, headers, 'apple');
    }
    logger.info('Deduplication: Using playlist name:', originalPlaylistName);
    
    // Now fetch all tracks without the 100 limit
    const { items } = await fetchApplePlaylistTrackIdsOrdered(playlist, headers);
    
    // Log the track count for debugging
    logger.info(`Deduplication: Original playlist "${originalPlaylistName}" has ${items.length} tracks`);
    
    // Log some sample tracks for debugging
    if (items.length > 0) {
      logger.info(`Deduplication: Sample tracks:`, items.slice(0, 3).map(t => `${t.name} - ${t.artist}`));
    }
    ...
    logger.info(`Deduplication: Found ${duplicateCount} duplicates, keeping ${catalogToAdd.length + libraryToAdd.length} unique tracks`);
    logger.info(`Deduplication: Processing ${catalogToAdd.length} catalog tracks and ${libraryToAdd.length} library tracks`);
    ...
    if (catalogToAdd.length > 0) {
      logger.info(`Deduplication: Adding ${catalogToAdd.length} catalog tracks to new playlist...`);
    ...
    if (libraryToAdd.length > 0) {
      logger.info(`Deduplication: Adding ${libraryToAdd.length} library tracks to new playlist...`);
    ...
    logger.error('dedupe_playlist failed:', error && error.response ? error.response.data : error.message);
...
app.post('/api/import-csv', async (req, res) => {
  try {
    ...
    if (service === 'spotify') {
      ...
      logger.info(`Starting parallel CSV processing of ${entries.length} songs in batches of ${BATCH_SIZE}...`);
      ...
      for (let batchStart = 0; batchStart < csvTracks.length; batchStart += BATCH_SIZE) {
        ...
        logger.info(`Processing CSV batch ${batchNumber}/${totalBatches} (${batch.length} songs)...`);
        ...
        batchResults.forEach(({ result, track, success, error }) => {
          if (success && result && result.success && result.match) {
            ...
            logger.info(`${confidence} CSV Matched: ${track.name} (${result.match.matchMethod}, ${result.match.matchTime}ms)`);
          } else {
            ...
            if (success) {
              logger.error(`CSV Not found: "${trackName}"${artistInfo}${albumInfo}`);
            } else {
              logger.error(`CSV Match failed for "${trackName}"${artistInfo}${albumInfo}: ${error}`);
            }
          }
        });
        
        logger.info(`CSV batch ${batchNumber}/${totalBatches} complete: ${batchResults.filter(r => r.success && r.result && r.result.success).length} matched, ${batchResults.filter(r => !r.success || !r.result || !r.result.success).length} not found`);
      }
      
      logger.info(`Parallel CSV processing complete: ${foundTracks} matched, ${notFound.length} unavailable`);
    ...
    logger.error('import-csv failed:', error && error.response ? error.response.data : error.message);
...
app.post('/api/export-csv', async (req, res) => {
  try {
    ...
    logger.error('export-csv failed:', error && error.response ? error.response.data : error.message);
...
app.post('/auto-sync/jobs', async (req, res) => {
  try {
    ...
    logger.error('Create auto-sync job failed:', error && error.message ? error.message : String(error));
...
app.post('/auto-sync/jobs/:id/run', async (req, res) => {
  ...
  setImmediate(async () => {
    try {
      ...
      logger.error('Manual job run failed:', e && e.message ? e.message : String(e));
...
app.post('/playlists/apple', async (req, res) => {
  ...
  try {
    ...
    logger.error('Apple Music fetch error:', err && err.response ? err.response.data : (err && err.message ? err.message : String(err)));
  ...
  setImmediate(async () => {
    ...
    try {
      ...
      for (let batchStart = 0; batchStart < tracks.length; batchStart += BATCH_SIZE) {
        ...
        logger.info(`DEBUG: Batch ${batchNumber} results:`, {
        ...
        batchResults.forEach(({ result, track, index, success, error }, forEachIndex) => {
          logger.info(`DEBUG forEach iteration ${forEachIndex + 1}/${batchResults.length}:`, {
            trackName: track.name,
            success,
            hasResult: !!result,
          ...
          if (success && result && result.success && result.match) {
            logger.info(`DEBUG: About to broadcast match for "${track.name}"`);
            if (existingDestIds.has(String(result.match.id))) {
              ...
              try {
                ...
                logger.error('Failed to write match to file:', fileError);
              }
              
              logger.info(msg); // Also log to console
              foundCount += 1; // Only count non-duplicates as actually found/added
            }
            logger.info(`DEBUG: Broadcasted match for "${track.name}"`);
          } else {
            notFoundCount += 1;
            const trackName = track.name || 'Unknown Track';
            const artistInfo = track.artists && track.artists.length > 0 ? ` by ${track.artists[0]}` : ' by Unknown Artist';
      ...
      if (uniqueToAdd.length > 0) {
        ...
        for (let i = 0; i < missingTracks.length; i++) {
          ...
          logger.error('Error adding tracks to Apple playlist:', error && error.response ? error.response.data : (error && error.message ? error.message : String(error)));
      ...
      logger.error('Sync job failed:', error && error.response ? error.response.data : (error && error.message ? error.message : String(error)));
    ...
    const fetchApplePlaylistTracksForFingerprinting_FIXED = async (playlistId, headers) => {
        ...
        try {
            ...
            if (error.response && error.response.status === 404) {
                logger.info('Primary playlist fetch failed with 404, trying fallback...');
                // Fallback Method
                try {
                    ...
                    logger.error('Fallback playlist fetch also failed:', fallbackError.response ? fallbackError.response.data : fallbackError.message);
    ...
    try {
      ...
      if (stillMissing.length > 0) {
        ...
        try {
          ...
          logger.warn('Library add during verification fallback failed:', libAddErr && libAddErr.message ? libAddErr.message : String(libAddErr));
        }
        // No delay - instant speed
        ...
        for (const id of stillMissing) {
          ...
          try {
            ...
            logger.warn('Verification fallback failed for id', id, vfErr && vfErr.message ? vfErr.message : String(vfErr));
          }
          // No delay - instant speed
        }
      }
      ...
      if (false) {
        try {
          ...
          logger.warn('Fix-order step failed:', e && e.message ? e.message : String(e));
      ...
      logger.error('Auto-Add Missing failed:', error.response ? error.response.data : error.message);
...
app.get('/api/logs', (req, res) => {
  try {
    ...
    logger.error('Failed to fetch log files:', error.message);
...
app.get('/api/logs/:filename', (req, res) => {
  try {
    ...
    // Log the access
    logger.info(`Log file accessed: ${filename} (${stats.size} bytes)`);
    
    res.setHeader('Content-Type', 'text/plain');
    res.setHeader('Content-Length', stats.size);
    res.send(content);
  } catch (error) {
    logger.error('Failed to fetch log content:', error.message);
    ...
    files.forEach(file => {
      if (file.endsWith('.log')) {
        ...
        try {
          ...
          logger.warn(`Failed to process log file ${file}:`, error.message);
        }
      }
    });
...
app.post('/api/logs/rotate', (req, res) => {
  try {
    ...
    logger.info(`Log rotation triggered manually - Old: ${oldFile}, New: ${currentLogFile}`);
    res.json({ message: 'Log rotated successfully', oldFile, newFile: currentLogFile });
  } catch (error) {
    logger.error('Failed to rotate log:', error.message);
...
app.get('/api/logs/status', (req, res) => {
  try {
    ...
    logger.error('Failed to get log status:', error.message);
...
app.post('/api/log-sync', (req, res) => {
  try {
    ...
    logger.info(syncLogMessage);
    
    if (details) {
      logger.info(`[SYNC_DETAILS] ${JSON.stringify(details)}`);
    }
  ...
  return new Promise((resolve, reject) => {
    const serverInstance = server.listen(port, '127.0.0.1', async () => {
      try {
        ...
        logger.info(`WebSocket server ready for real-time progress updates`);
        logger.info('Startup optimizations enabled: Apple Music token pre-fetching and service pre-warming');
        ...
        if (hasEnvToken || hasStoredToken) {
          ...
          logger.info('💡 Apple Music developer token not found. Use the app UI to set it up when needed.');
        }
        ...
        setImmediate(async () => {
          ...
          if (storedToken && storedToken !== 'your_token_here') {
            ...
            getDeveloperToken(false).catch(error => {
              logger.info('Apple Music developer token initialization failed, but server will continue:', error.message);
            });
          } else {
            logger.info('💡 Apple Music developer token not found. Will be set up when user tries to use Apple Music features.');
            ...
            try {
              ...
              if (!canLaunch) {
                logger.info('Puppeteer dependencies missing. Will install automatically when needed.');
              } else {
                logger.info('Puppeteer dependencies are available.');
              }
            } catch (error) {
              logger.warn('Could not check Puppeteer dependencies:', error.message);
            }
          }
        });
